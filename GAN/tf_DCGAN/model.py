...
class DCGAN():
    def __init__(self, sess, input_height=108, input_width=108, crop=True, batch_size=64,
                 sample_num=64, output_height=64, output_width=64, y_dim=None, z_dim=100, gf_dim=64,
                 df_dim=64, gfc_dim=1024, dfc_dim=1024, c_dim=3, dataset_name="default",
        ...
        if self.dataset_name == "mnist":
            ...
            self.c_dim = self.data_X[0].shape[-1] # 最后一个维度表示图片输入的通道数
        else:
            data_path = os.path.join(self.data_dir, self.dataset_name, self.input_fname_pattern)
            self.data = glob(data_path) # 返回查找的所有符合文件路径列表
            if len(self.data) == 0:
                raise Exception(" No data found in '"+data_path+"'")
            np.random.shuffle(self.data)
            imreadImg = imread(self.data[0])
            if len(imreadImg.shape) >= 3: # 检查图片是否是 non-grayscale
    ...
    def generator(self, z, y=None):
        with tf.variable_scope("generator") as scope:
            if not self.y_dim:
                ...
                self.h3, self.h3_w, self.h3_b = deconv2d(h2, [self.batch_size, s_h2, s_w2, self.gf_dim*1],
                                                         name='g_h3', with_w=True)
                h3 = tf.nn.relu(self.g_bn3(self.h3))
                self.h4, self.h4_w, self.h4_b = deconv2d(h3, [self.batch_size, s_h, s_w, self.c_dim],
    ...
    def build_model(self):
        ...
        self.sampler = self.sample(self.z, self.y)
            ...
            for idx in range(0, int(batch_idxs)):
                ...
                if counter % 100 == 1:
                    if config.dataset == "mnist":
                        ...
                        save_images(samples, image_manifold_size(samples.shape[0]),
                                    './{}/train_{:02d}_{:04d}.png'.format(config.sample_dir, epoch, idx))
                        print("Sample d_loss: %.6f, g_loss: %.6f" % (d_loss, g_loss))
                if counter % 500 == 2:
