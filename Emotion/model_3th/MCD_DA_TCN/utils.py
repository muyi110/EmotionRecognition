...
def batch_generator(X, y, X_test, y_test, batch_size, seed):
    ...
    num_complete_minibatches = num // (batch_size//2)
    minibatchs = []
    for k in range(num_complete_minibatches):
        X_s = shuffled_X[k*(batch_size//2):(k+1)*(batch_size//2)]
        y_s = shuffled_y[k*(batch_size//2):(k+1)*(batch_size//2)]
        X_t = shuffled_X_test[k*(batch_size//2):(k+1)*(batch_size//2)]
        y_t = shuffled_y_test[k*(batch_size//2):(k+1)*(batch_size//2)]
