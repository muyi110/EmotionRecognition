...
class DANNModel():
    ...
    def _build_model(self, n_outputs):
        ...
        with tf.variable_scope("domain_predictor"):
            ...
            d_temp = tf.contrib.layers.fully_connected(feat, 30, activation_fn=tf.nn.leaky_relu)
            # d_temp_0 = tf.contrib.layers.dropout(d_temp, 1-0.7, is_training=self._training)
            # d_temp1 = tf.contrib.layers.fully_connected(d_temp, 20, activation_fn=tf.nn.leaky_relu)
            d_logits = tf.contrib.layers.fully_connected(d_temp, 1, activation_fn=None)
            self.domain_predictor = tf.sigmoid(d_logits)
    ...
    def _build_graph(self, n_outputs):
        ...
        # 构建训练节点
        pre_train_op = self.optimizer_class1(learning_rate=self.learning_rate).minimize(pred_loss)
        dann_train_op = self.optimizer_class1(learning_rate=self.learning_rate).minimize(total_loss)
        domain_train_op = self.optimizer_class2(learning_rate=self.learning_rate).minimize(domain_loss_)
        # 构建评估节点
                ...
                if training_mode == "dann":
                    ...
                    if pre_train_flag:
                        sess.run(self._regular_train_op, feed_dict={self.inputs:X_batch, self.labels:y_batch, 
                                                                    self.domain_labels:domain_label, self._training:True, 
                                                                    self.l:l, self.learning_rate:lr})
                    elif domain_train_flag:
                        sess.run(self._domain_train_op, feed_dict={self.inputs:X_batch, self.labels:y_batch, 
                                                                   self.domain_labels:domain_label, self._training:True, 
                                                                   self.l:l, self.learning_rate:lr})
                    # print info
                    if step % (len(y)//(self.batch_size*self.train_ratio)) == 0 and step != 0:
                        ...
                        if pre_train_flag:
                            train_pred_total_loss, train_pred_acc = sess.run([self._pred_loss, self._label_acc], 
                                                                              feed_dict={self.inputs:X, self.labels:y})
                            if train_pred_total_loss > 0.2:
                                pre_train_flag = True
                                domain_train_flag = False
                            else:
                                pre_train_flag = False
                                domain_train_flag = True
                        ...
                        if pre_train_flag:
                            print("{}\ttraining loss: {:.4f}\t|  trainning accuracy: {:.2f}%".format(epoch_count, 
                                  train_pred_total_loss, train_pred_acc*100))
                        if domain_train_flag:
                            domain_loss, domain_acc, p = sess.run([self._domain_loss, self._domain_acc, 
                                                                   self.domain_predictor], 
                                                                   feed_dict={self.inputs:X_batch, self.labels:y_batch, 
                                                                   self.domain_labels:domain_label})
                            if domain_acc > 0.6 or domain_acc < 0.4:
                                pre_train_flag = False
                                domain_train_flag = True
                            else:
                                pre_train_flag = True
