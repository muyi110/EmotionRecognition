...
class TCN_Model():
    def __init__(self, input_layer, output_size, num_channels, sequence_length, 
                 kernel_size, dropout, is_training, scope="TCN_Net", reuse=False, test_flag=False):
    ...
    def _TCN(self):
        with tf.variable_scope(self.scope, reuse=self.reuse):
           ...
           self.feature_test = features[:, -1, :]
           feature_one = tf.contrib.layers.fully_connected(features[:, -1, :], 32, 
                                                           activation_fn=tf.nn.relu, scope="feature_one")
           self.feature = tf.contrib.layers.fully_connected(feature_one, 32, 
                                                            activation_fn=tf.nn.relu, scope="feature")
           self.feature = self.feature + features[:, -1, :]
           feature_layer = tf.cond(tf.equal(self.test_flag, True), lambda:self.feature_test, lambda:self.feature)
           self.logits = tf.contrib.layers.fully_connected(feature_layer, self.output_size, 
