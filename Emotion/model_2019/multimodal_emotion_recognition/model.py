...
class MMJLNNModel(): # 多模态联合学习神经网络
    ...
    def _build_model(self, n_outputs):
        ...
        # PER
        self.EOG_inputs = tf.placeholder(tf.float32, shape=(None, self.n_steps, 20), name="EOG_inputs")
        self.EMG_inputs = tf.placeholder(tf.float32, shape=(None, self.n_steps, 20), name="EMG_inputs")
        self.GSR_inputs = tf.placeholder(tf.float32, shape=(None, self.n_steps, 10), name="GSR_inputs")
        self.RSP_inputs = tf.placeholder(tf.float32, shape=(None, self.n_steps, 10), name="RSP_inputs")
        self.BLV_inputs = tf.placeholder(tf.float32, shape=(None, self.n_steps, 10), name="BLV_inputs")
        self.TMR_inputs = tf.placeholder(tf.float32, shape=(None, self.n_steps, 10), name="TMR_inputs")
        # EEG
        ...
        with tf.variable_scope("EOG_feature_extractor"):
            ...
            #EOG_outputs, EOG_states = tf.nn.dynamic_rnn(EOG_lstm_cell, self.EOG_inputs, dtype=tf.float32)
        ...
        with tf.variable_scope("EMG_feature_extractor"):
            ...
            #EMG_outputs, EMG_states = tf.nn.dynamic_rnn(EMG_lstm_cell, self.EMG_inputs, dtype=tf.float32)
        ...
        with tf.variable_scope("GSR_feature_extractor"):
            ...
            #GSR_outputs, GSR_states = tf.nn.dynamic_rnn(GSR_lstm_cell, self.GSR_inputs, dtype=tf.float32)
        ...
        with tf.variable_scope("RSP_feature_extractor"):
            ...
            #RSP_outputs, RSP_states = tf.nn.dynamic_rnn(RSP_lstm_cell, self.RSP_inputs, dtype=tf.float32)
        ...
        with tf.variable_scope("BLV_feature_extractor"):
            ...
            #BLV_outputs, BLV_states = tf.nn.dynamic_rnn(BLV_lstm_cell, self.BLV_inputs, dtype=tf.float32)
        ...
        with tf.variable_scope("TMR_feature_extractor"):
            ...
            #TMR_outputs, TMR_states = tf.nn.dynamic_rnn(TMR_lstm_cell, self.TMR_inputs, dtype=tf.float32)
        ...
        #self.concat_feature = self.feature
        with tf.variable_scope("classifier"):
            concat_logit_1 = tf.contrib.layers.fully_connected(self.concat_feature, 100, activation_fn=tf.nn.relu)
            concat_logit_1_drop = tf.contrib.layers.dropout(concat_logit_1, 0.5, is_training=self._training) 
    ...
    def _build_graph(self, n_outputs):
        ...
        total_loss = predictor_loss + domain_loss + feature_loss*0.1
