...
def _save_samples(datas_result, labels, people_list, classify_object_name):
    ...
    if len(people_list) == 1:
        ...
        np.save("./peripheral_features/"+class_name+"/s"+str(people_list[0])+"/datas", datas_result)
        np.save("./peripheral_features/"+class_name+"/s"+str(people_list[0])+"/labels", labels)

if __name__ == "__main__":
    people_list = list(range(0, 32))
    trial_list = list(range(0, 40))
    datas, labels = get_data(people_list, trial_list, classify_object_name=1)
    ...
    for people in range(32):
        ...
        for i in range(60):
            data_1s = peripheral_data[:, :, i*128:(i+1)*128]
            assert(data_1s.shape == (40, 8, 128))
            for j in range(40):
                for k in range(8):
                    # 将数据进行归一化处理
                    max_value = max(data_1s[j, k, :])
                    min_value = min(data_1s[j, k, :])
                    data_normalization = (data_1s[j, k, :] - min_value) / (max_value - min_value + 1e-8)
                    assert(data_normalization.shape == (128, ))
                    samples.append(data_normalization)
        samples = np.array(samples)
        assert(samples.shape == (19200, 128))
        ...
        with autoencoder._session.as_default() as sess:
            ...
            print("loss: ", loss)
            assert(features.shape == (19200, 10))
            # 将特征转为 shape=(40, 80, 60)
            features_result = []
            for p in range(40):
                temp_list = []
                for q in range(60):
                    temp_list.append(features[p*8+320*q:(p+1)*8+320*q, :].reshape(80, 1))
                features_result.append(np.hstack(temp_list))
            features_result = np.array(features_result)
            assert(features_result.shape == (40, 80, 60))
            labels_one_people = labels[people*40:(people+1)*40]
            assert(labels_one_people.shape == (40, ))
            _save_samples(features_result, labels_one_people, [people], 1)
