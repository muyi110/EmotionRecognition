    ...
    with tf.variable_scope(name):
        ...
        if in_channels != out_channels:
            W_h = tf.get_variable('W_h', [1, int(input_layer.get_shape()[-1]), out_channels], 
                                  tf.float32, init_weights, trainable=True)
...
def TCN(input_layer, output_size, num_channels, sequence_length, kernel_size, dropout, is_training=True):
    tcn_ = temporal_ConvNet(input_layer=input_layer, num_channels=num_channels, sequence_length=sequence_length, 
                           kernel_size=kernel_size, dropout=dropout, is_training=is_training)
    # tcn shape = (samples, seq_length, features)
    # 利用投影矩阵，将 tcn 的输出进一步压缩
    # tcn = tf.transpose(tcn_, perm=[0,2,1])
    # bias = tf.get_variable("bias", shape=(tcn.get_shape()[1].value, 1), initializer=tf.constant_initializer(1.0))
    # project_matrix_1 = tf.random_uniform((tcn.get_shape()[2].value, 3))
    # temp = [project_matrix_1]*8
    # project_matrix = tf.Variable(temp, name="project_matrix")
    # tcn_output = tf.add(tf.matmul(tcn, project_matrix), bias, name="tcn_output")
    # tcn_sigmoid = tf.nn.sigmoid(tcn_output, name="tcn_sigmoid")
    # tcn_sigmoid = tf.layers.flatten(tcn_sigmoid)
    # linear = tf.contrib.layers.fully_connected(tcn_sigmoid, output_size, activation_fn=None)
    linear = tf.contrib.layers.fully_connected(tcn_[:, -1,:], output_size, activation_fn=None)
    return linear
