...
class TCNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _bulid_graph(self, n_outputs):
        ...
        # predictions = tf.nn.sigmoid(tcn_outputs, name="predictions") # 2 分类
        # 计算交叉熵
        xentropy = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels, logits=tcn_outputs)
        # xentropy = tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.cast(labels, tf.float32), logits=tcn_outputs) # 2分类
        ...
        # accuracy = tf.reduce_mean(tf.cast((tf.cast((predictions >= 0.5),tf.int32) == labels), tf.float32))
        # 构建全局初始化节点和模型保存节点
...
if __name__ == "__main__":
    ...
    train_index = permutation[2:3]
    print("people: ", train_index)
    test_index = permutation[2:3]
    ...
    datas_train, train_labels = read_data(people_list=train_index, sample_flag=True, 
                                          trial_list=train_trial_list, windows=9, overlapping=8, raw_data=True)
    datas_test, test_labels = read_data(people_list=test_index, sample_flag=False, 
                                        trial_list=test_trial_list, windows=9, overlapping=8, raw_data=True)
    ...
    dropout = 0.0
    learning_rate=0.001
    num_channels = [120, 120, 120] # 有多少层，及每一层包含的神经元个数（这里的一层指一个 block）
    kernel_size = 3   # 卷积核大小  
    batch_size = 64
