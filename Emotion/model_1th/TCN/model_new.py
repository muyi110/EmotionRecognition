...
from tcn import TCN
from data_new import read_data, index_generator
...
if __name__ == "__main__":
    # 开始将数据集划分为训练集和测试集
    np.random.seed(42)
    permutation = list(np.random.permutation(32)) # 将数据随机打乱
    train_index = permutation[3:4]
    test_index = permutation[3:4]
    train_trial_list = list(range(0, 32))
    test_trial_list = list(range(32, 40))
    # 获取生理信号数据
    datas_train, train_labels = read_data(people_list=train_index, 
                                          trial_list=train_trial_list, windows=9, overlapping=8, raw_data=False)
    datas_test, test_labels = read_data(people_list=test_index, 
    ...
    num_channels = [120, 120] # 有多少层，及每一层包含的神经元个数（这里的一层指一个 block）
    kernel_size = 5   # 卷积核大小  
    batch_size = 16
